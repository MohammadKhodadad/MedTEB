...
output_file=f"../data/pair_classification/wiki_diseases_dataset.json"
if os.path.exists(output_file):
    print(f"{output_file} already exists")
else:
    print(f"Downloading data for task: Diseases and disorders by system")
    wiki_create_pair_classification_data(["Diseases and disorders by system"],1500,max_depth=1,output_file=output_file)
...
for t1,t2 in tasks:
    ...
    output_file=f"../data/pair_classification/mimic_{t1.replace(' ','_')}_vs_{t2.replace(' ','_')}.json"
    if os.path.exists(output_file):
        print(f"{output_file} already exists")
    else:
        print(f"Creating data for task: {t1} vs {t2}")
        mimic_create_pair_classification_dataset('./dataloaders/data/discharge_processed_v_3.csv',t1,t2,output_file,1500)
...
output_file=f"../data/pair_classification/medqa_pair_classification.json"
if os.path.exists(output_file):
    print(f"{output_file} already exists")
else:
...
output_file=f"../data/pair_classification/pubmed_pair_classification.json"
if os.path.exists(output_file):
    print(f"{output_file} already exists")
else:
    print(f"Loading Data for task: pubmed pairclassification")
    pubmed_create_pair_classification_data(tasks,1500,output_file)
