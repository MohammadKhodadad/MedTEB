...
def create_retrieval_task(data_address):
    class CustomAbsTaskRetrieval(AbsTaskPairClassification):
        ...
        def load_data(self, **kwargs: Any) -> None:
            ...
            for i in range(len(data)):
                    row = data.iloc[i]
                    self.dataset["test"]['sentence1'][0] .append(row['sentence1'])
                    self.dataset["test"]['sentence2'][0] .append(row['sentence2'])
                    self.dataset["test"]['labels'][0] .append(row['label'])
...
if __name__ == "__main__":
    task = create_retrieval_task('..\\data\\pair_classification\\mimic_Chief_Complaint_vs_Discharge_Diagnosis.csv')
